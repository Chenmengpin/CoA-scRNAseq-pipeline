if __name__ == '__main__':  # required to run outside jupyter notebook
    ...
    tf_raw = pd.read_csv("TF_import.txt", delimiter = "\t",     # imports in the raw gene annotation information from TFCat 
    ...
    conv_tfs = pd.read_csv("TF_conversion.txt", delimiter = "\t")   # imports in MGI gene names from DAVID
    ...
    ex_matrix = pd.read_csv("GENIE3_import.csv", header = 0, index_col = 0).T    # loads expression matrix, make sure you transpose back
    databases_glob = os.path.join("mm10__*.feather") # loads cisTarget databases into memory
